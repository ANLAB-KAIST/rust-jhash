...
{
	return jhash_nwords(a, b, c, initval.wrapping_add(JHASH_INITVAL).wrapping_add(3 << 2));
}

#[inline(always)]
pub fn jhash_2words(a: u32, b: u32, initval: u32) -> u32
{
	return jhash_nwords(a, b, 0, initval.wrapping_add(JHASH_INITVAL).wrapping_add(2 << 2));
}

#[inline(always)]
pub fn jhash_1words(a: u32, initval: u32) -> u32
{
	return jhash_nwords(a, 0, 0, initval.wrapping_add(JHASH_INITVAL).wrapping_add(1 << 2));
}
...
impl std::hash::Hasher for JHasher {
    ...
    fn finish(&self) -> u64 {
    	match self.buffer {
			...
			JHashBuffer::One(val1) => {
				jhash_1words(val1, self.current) as u64
			},
			JHashBuffer::Two(val1, val2) => {
